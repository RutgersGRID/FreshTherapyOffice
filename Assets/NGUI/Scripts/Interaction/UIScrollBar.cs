...
public class UIScrollBar : MonoBehaviour
{
	...
	{
		MouseLook cameraController = GameObject.Find("localPlayer").GetComponent<MouseLook>(); //this is to prevent the camera from moving when we drag around a scrollbar
		cameraController.GUIInputSelected=isPressed;
		if (isPressed)
		{
			mCam = UICamera.currentCamera;
