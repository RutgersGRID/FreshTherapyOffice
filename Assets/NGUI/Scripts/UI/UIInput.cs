...
public class UIInput : MonoBehaviour
{
	...
	private FPSInputController inputController; //RSO
	private MouseLook cameraController;
	private AnimateCharacter animationController;
	...
	{
		ChatInput.mIgnoreNextEnter=isSelected;
		if(inputController==null)
		{
			inputController = GameObject.Find("localPlayer").GetComponent<FPSInputController>(); //RSO
			cameraController = GameObject.Find("localPlayer").GetComponent<MouseLook>();
			animationController = GameObject.Find("localPlayer").GetComponentInChildren<AnimateCharacter>();
		}
		animationController.GUIInputSelected=isSelected;
		cameraController.GUIInputSelected=isSelected;
		if(!Input.GetKey(KeyCode.Return))
		{
			FPSInputController.controlsEnabled=!isSelected;
		}
		//toggle the input lock of the text area
	...
	void OnInput (string input)
	{
		if (selected && enabled && gameObject.active)
		{
			...
			if(cursorPos>mText.Length)
			{
				Debug.Log("Cursor went past end of line - moving back to end of line");
				cursorPos=mText.Length;
			}
			...
			for (int i = 0, imax = input.Length; i < imax; ++i)
			{
				...
				if (c == '\b')
				{
					...
					if (cursorPos > 0)
					{
						mText=mText.Substring(0,cursorPos-1)+mText.Substring(cursorPos);
						cursorPos--;
//						mText = mText.Substring(0, mText.Length - 1);
				...
				else if (c == '\r' || c == '\n')
				{
					...
					eventReceiver.SendMessage(functionName, SendMessageOptions.DontRequireReceiver);
					current = null;
/*					c = '\r'; //we always use \r because \n is reserved for newlines inserted by ngui
					if(mText.Length!=cursorPos)
					{
						mText = mText.Substring(0,cursorPos) + c + mText.Substring(cursorPos);
					}
					else
					{
						mText+=c;
					}*/
//					selected = false; //unselect the chat window when we hit enter
					return;
				}
				else if (c >= ' ')
				{
	...
	void OnPress(bool isPressed)
	{
		mText=label.text;
		mText=mText.Replace("|", "");
		Vector3 positionClicked = UICamera.currentCamera.ScreenToWorldPoint(Input.mousePosition);
		positionClicked=positionClicked-label.transform.position;
		...
		int lineWhereCharacterShouldBe = -(int)(positionClicked.y/label.transform.lossyScale.y); //since we are anchored to the top right we have to multiply by -1
		if(lineWhereCharacterShouldBe<lines.Length && lineWhereCharacterShouldBe>=0)
		{
			float percentFilled = label.font.GetTextLength(lines[lineWhereCharacterShouldBe], false, UIFont.SymbolStyle.None)/(label.font.size*label.lineWidth / label.cachedTransform.localScale.x);
			if(percentFilled!=0)
			{
				int indexWhereCharacterShouldBe = (int)(lines[lineWhereCharacterShouldBe].Length*(positionClicked.x/(label.GetComponent<Collider>().bounds.extents.x*2))/percentFilled); //this will the index in the string where we clicked on a particular line, if we clicked past the text the value will be greater than the length of the string
				if(indexWhereCharacterShouldBe<lines[lineWhereCharacterShouldBe].Length)
				{
					char selectedChar = lines[lineWhereCharacterShouldBe][indexWhereCharacterShouldBe];
					cursorPos=0;
					for(int i = 0; i<lineWhereCharacterShouldBe; i++)
					{
						cursorPos+=lines[i].Length+1;
					}
					cursorPos+=indexWhereCharacterShouldBe;
					//move the cursor
				}
				else
				{
					//we clicked on the whitespace between the last character on the line and the actual end of the line, so we move to the end of the current line.
					cursorPos=0;
					for(int i = 0; i<lineWhereCharacterShouldBe; i++)
					{
						cursorPos+=lines[i].Length+1;
					}
					cursorPos+=lines[lineWhereCharacterShouldBe].Length-1;
				}
			}
		}
		else
		{
			cursorPos=mText.Length; //we clicked past the end so we move the cursor to the end
		}
		UpdateLabel();
	}
	void UpdateLabel ()
	{
		if(mText.Length<cursorPos)
		{
			Debug.LogError("Cursor has somehow been moved to an impossible position");
			cursorPos=mText.Length;
		}
		...
		if (label.font != null)
		{
			...
			if(mText.Length!=cursorPos && cursorPos!=0)//chop out the old cursor so that we don't have multiple cursors
			{
				processed = selected ? (mText.Substring(0,cursorPos) + Input.compositionString + caratChar+mText.Substring(cursorPos)) : mText;
			}
			else if(cursorPos==0)
			{
				processed = selected ? (caratChar+mText) : mText;
			}
			else
			{
			...
			int oldLength = processed.Length;
//			processed = label.font.WrapText(processed, label.lineWidth / label.cachedTransform.localScale.x, true, false, UIFont.SymbolStyle.None);
			if (!label.multiLine)
			{
